...
public class DoublyLinkedList {
	...
	public void insertTail(int x) {
		...
		if (isEmpty()) { // Check is there are no elements in list then it adds first element
			tail = newLink;
			head = tail;
		} else {
	 ...
	 *
	 * @param x Element to be inserted
	 * @parem index Index(from start) at which the element x to be inserted
	 */
	...
	public Link deleteHead() {
		...
		if (head == null) {
			...
			head.previous
					= null; // oldHead --> 2ndElement(head) nothing pointing at old head so will be removes
		}
		--size;
		return temp;
	}
	...
	public static void removeDuplicates(DoublyLinkedList l) {
		...
		while (linkOne.next != null) { // list is present
			...
			linkOne = linkOne.next; // got to link to iterate the whole list again
		}
	}
